...
export const usePipelineStore = defineStore("pipeline", {
  ...
  actions: {
    ...
    moveFunctionalStep(id, direction) {
      ...
      if (direction === "up" && index > 0) {
        [this.functionalSteps[index], this.functionalSteps[index - 1]] = [
          this.functionalSteps[index - 1],
          this.functionalSteps[index],
        ];
      } else if (
        direction === "down" &&
        index < this.functionalSteps.length - 1
      ) {
        [this.functionalSteps[index], this.functionalSteps[index + 1]] = [
          this.functionalSteps[index + 1],
          this.functionalSteps[index],
    ...
    async executePipeline() {
      ...
      try {
        // Import XJX library
        const { XJX, toNumber, toBoolean, regex } = await import(
          "../../../dist/esm/index.js"
        );

        // Create pipeline hooks if enabled
        const pipelineHooks = this.enablePipelineHooks
          ? this.createPipelineHooks()
          : undefined;
        ...
        if (pipelineHooks) {
          builder = new XJX(configStore.config, pipelineHooks).withLogLevel(
        ...
        for (const step of this.allSteps) {
          builder = this.applyStep(builder, step, {
            toNumber,
            toBoolean,
            regex,
          });
    ...
    createPipelineHooks() {
      ...
      if (options.logSteps || options.logTiming || customBeforeStepFn) {
        hooks.beforeStep = async (stepName, input) => {
          ...
          if (options.logSteps) {
            if (!logger) {
              const { LoggerFactory } = await import(
                "../../../dist/esm/index.js"
              );
          ...
          if (customBeforeStepFn) {
            try {
              ...
              console.warn(
                `Error in custom beforeStep hook for ${stepName}:`,
                err
              );
            }
          }
        ...
        hooks.afterStep = async (stepName, output) => {
          ...
          if (customAfterStepFn) {
            try {
              ...
              console.warn(
                `Error in custom afterStep hook for ${stepName}:`,
                err
              );
            }
          }
...
applyStep(builder, step, transforms) {
    ...
    async executeTerminalOperation(builder) {
      ...
      switch (type) {
        ...
        case "toCsv":
          return builder.toCsv(outputHooks);
        case "toXnode": {
