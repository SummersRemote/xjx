...
export class DefaultXNodeToJsonConverter implements XNodeToJsonConverter {
  ...
  private elementToJson(node: XNode): Record<string, any> {
    ...
    const nodeName = this.config.preserveNamespaces && node.prefix 
      ...
      switch (child.type) {
        ...
        case NodeType.CDATA_SECTION_NODE:
          ...
          if (this.config.cdataAsText) {
            ...
            result.push({ [this.config.propNames.cdataKey]: child.value || "" });
          }
          break;
        ...
        case NodeType.COMMENT_NODE:
          // Comment node
          if (this.config.commentMode === "preserve") {
            result.push({ [this.config.propNames.commentKey]: child.value || "" });
          }
          break;
      }
    }
  ...
  private processChildElements(node: XNode): Record<string, any> {
    ...
    for (const child of node.children) {
      if (child.type === NodeType.ELEMENT_NODE) {
        const key = this.config.preserveNamespaces && child.prefix 
      ...
      } else if (child.type === NodeType.COMMENT_NODE && this.config.commentMode === "preserve") {
        // Handle comments
    ...
    for (const [key, elements] of elementsByName.entries()) {
      ...
      if (elements.length > 1 || this.repeatedElements.has(key) || this.config.alwaysUseArrays) {
  ...
  private analyzeNodeContent(node: XNode): {
    ...
    const hasDirectValue = node.value !== undefined && typeof node.value !== 'object';
   ...
   * @param node Node with text content
   * @returns Normalized text content
   */
  private getNodeTextContent(node: XNode): string {
    // Check for direct value
    if (node.value !== undefined && typeof node.value !== 'object') {
      return this.normalizeText(String(node.value));
    }
    ...
    const text = node.children
      .filter(child => 
        ...
        (this.config.cdataAsText && child.type === NodeType.CDATA_SECTION_NODE)
      )
