...
export class XJX {
   ...
   */
  public transformValue(direction: TransformDirection, transformer: ValueTransformer): this {
   ...
   * @returns This XJX instance for chaining
   */
  public transformAttribute(direction: TransformDirection, transformer: AttributeTransformer): this {
   ...
   * @returns This XJX instance for chaining
   */
  public transformChildren(direction: TransformDirection, transformer: ChildrenTransformer): this {
   ...
   * @returns This XJX instance for chaining
   */
  public transform(direction: TransformDirection, transformer: NodeTransformer): this {
   ...
   */
  public applyValueTransformers(value: any, node: XNode, context: TransformContext): any {
  ...
  public applyNodeTransformers(node: XNode, context: TransformContext): XNode {
    ...
    for (const transformer of transformers) {
      ...
      if (result) transformedNode = result;
    }
  ...
  public applyTransformations(node: XNode, context: TransformContext): XNode | null {
    ...
    if (node.value !== undefined) {
      const transformedValue = this.applyValueTransformers(node.value, node, context);
    ...
    if (node.attributes) {
      ...
      for (const [name, value] of Object.entries(node.attributes)) {
        ...
        const attrContext: TransformContext = this.transformUtil.createAttributeContext(
          context,
          name
        );
    ...
    if (transformedNode === null) return null;
    ...
    if (transformedNode.children) {
      ...
      if (transformedNode.children.length > 0) {
        transformedNode.children = transformedNode.children
          .map((child, index) => {
            const childContext: TransformContext = this.transformUtil.createChildContext(
              context,
              child,
              index
            );
   ...
   */
  public getValueTransformers(direction: TransformDirection): ValueTransformer[] {
   ...
   * @internal For use by converters
   */
  public getAttributeTransformers(direction: TransformDirection): AttributeTransformer[] {
   ...
   * @internal For use by converters
   */
  public getChildrenTransformers(direction: TransformDirection): ChildrenTransformer[] {
