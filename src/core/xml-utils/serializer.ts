/**
 * XML serialization utilities with hybrid OO-functional approach
 * 
 * Provides serialization capabilities for XML nodes and documents
 * with a clean interface and pure functional core.
 */
import { DOM } from '../dom';
import { logger, validate, handleError, ErrorType, SerializeError } from '../error';
import { XmlEntity } from './entity';

/**
 * XML serializer with static methods for easier integration
 */
export class XmlSerializer {
  ...
  static serialize(node: Node): string {
    try {
      ...
      return serializeNodeToString(node);
    } catch (err) {
  ...
  static prettyPrint(xmlString: string, indent: number = 2): string {
    try {
      ...
      // Use pure function for formatting
      return formatXml(xmlString, indent);
    } catch (err) {
      return handleError(err, 'pretty print XML', {
        ...
        errorType: ErrorType.SERIALIZE,
        fallback: xmlString // Return original string as fallback
      });
    }
  }
  ...
  static ensureXMLDeclaration(xmlString: string): string {
    try {
      ...
      // Use pure function to add declaration
      return addXmlDeclaration(xmlString);
    } catch (err) {
  ...
  static createEmptyDocument(): Document {
    try {
      // Use pure function to create document
      return createEmptyXmlDocument();
    } catch (err) {
      return handleError(err, 'create empty XML document', {
        errorType: ErrorType.SERIALIZE,
        fallback: DOM.createDocument() // Return basic document as fallback
      });
    }
  }
}
...
/**
 * Serialize DOM node to XML string - pure function
 * @param node DOM node to serialize
 * @returns Serialized XML string
 */
export function serializeNodeToString(node: Node): string {
...
/**
 * Format XML string with indentation - pure function
 * @param xmlString XML string to format
...
/**
 * Add XML declaration to a string if missing - pure function
 * @param xmlString XML string
 * @returns XML string with declaration
 */
export function addXmlDeclaration(xmlString: string): string {
...
/**
 * Create an empty DOM document - pure function
 * @returns New empty DOM document
 */
